...
namespace VRC.SDKBase.Editor
{
    public class VRCSdkControlPanelAvatarBuilder : IVRCSdkControlPanelBuilder
    {
        ...
        public void ShowSettingsOptions()
        {
            ...
            EditorGUILayout.LabelField("Avatar", Style.Label);
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.Separator();

            bool prevShowPerfDetails = showAvatarPerformanceDetails;
            bool showPerfDetails = EditorGUILayout.Toggle(new GUIContent("Performance", "Show all avatar performance details."), prevShowPerfDetails);
        ...
        public virtual void ShowBuilder()
        {
            if (_avatars.Length > 0)
            {
                ...
                if (drawList)
                {
                    ...
                    _avatarListScrollPos = GUILayout.BeginScrollView(_avatarListScrollPos, false, false, GUIStyle.none, Style.Scroll, Style.Width);
        ...
        public virtual void OnGUIAvatarCheck(VRC_AvatarDescriptor avatar)
        {
            ...
            if (!string.IsNullOrEmpty(vrcFilePath) && Bloodborne.VRChat.CheckIfAssetBundleFileTooLarge(ContentType.Avatar, vrcFilePath, out int fileSize, isMobilePlatform))
            {
                EditorPrefs.DeleteKey("currentBuildingAssetBundlePath");
            }

            _builder.OnGUIWarning(
                avatar,
                Bloodborne.VRChat.Warning.Avatar(),
                delegate { Selection.activeObject = avatar.gameObject; },
                null
            );

            int Dynamic = Bloodborne.VRChat.Dynamic.Count(avatar.gameObject);

            if (Dynamic > 0)
            {
                Action find = delegate () { Bloodborne.VRChat.Dynamic.Find(avatar.gameObject); Selection.activeObject = avatar.gameObject; };
                Action remove = delegate () { Bloodborne.VRChat.Dynamic.Remove(avatar.gameObject); };
                _builder.OnGUIWarning(avatar, Bloodborne.VRChat.Dynamic.Text(Dynamic), find, remove);
            }
#endif

            int Corruption = Bloodborne.VRChat.Corruption.Count(avatar.gameObject);

            if (Corruption > 0)
            {
                Action find = delegate () { Bloodborne.VRChat.Corruption.Find(avatar.gameObject); Selection.activeObject = avatar.gameObject; };
                Action remove = delegate () { Bloodborne.VRChat.Corruption.Remove(avatar.gameObject); };
                _builder.OnGUIWarning(avatar, Bloodborne.VRChat.Corruption.Text(Corruption), find, remove);
            }
            ...
            //         () =>
            //         {
            //             EditorApplication.ExecuteMenuItem("Bloodborne/Utility/Avatar Shader Keywords Utility");
            //         });
...
// #endif
        ...
        private static void OnGUIAvatarSettings(VRC_AvatarDescriptor avatar)
        {
            ...
            if (avatar.apiAvatar != null)
            {
                ...
                Bloodborne.VRChat.Label.Avatar();
